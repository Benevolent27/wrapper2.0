...
if [ "${2}" ]; then
	...
	playerFactionIni="$(echo "${scriptCalled}" | sed 's/\.sh$//g').ini"
	# echo "playerFactionIni: ${playerConnectIni}"
	runScript "$@"
else
	echo "Usage: playerDisconnectLoader.sh \"\PATH\TO\SCRIPT\playerFaction.sh\" [PlayerName] [action] [factionID] [factionName]"
	echo "Note, the action's can be either playerJoin or playerLeave"
fi
