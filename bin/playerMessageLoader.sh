...
# Turn off globbing mode so that * characters don't get interpreted
set -f
...
if [ "${2}" ]; then
	...
	name="$1"
	shift
	receiverType=$(echo "$1" | tr '[:upper:]' '[:lower:]')
	shift
	receiver="$1"
	shift
	message="$@"
	...
	function runScript {
		...
		echo "Running playerMessage Script: ${scriptCalled} ${@}"
		. "${scriptCalled}" "${@}"
	}

	# First check to see if the script is an admin-only script or not, and only run it if the person is an admin if so.
	playerMessageIni="$(echo "${scriptCalled}" | sed 's/\.sh$//g').ini"
	# echo "playerMessageIni: ${playerMessageIni}"
	runScript "$@"
else
	echo "Usage: playerMessageLoader.sh \"\PATH\TO\SCRIPT\playerMessage.sh\" [PlayerName] [receiverType] [receiver] [message]"
fi
