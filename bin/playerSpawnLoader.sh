...
if [ "${2}" ]; then
	...
	function runScript {
		...
		echo "Running playerSpawn script: ${scriptCalled} ${@}"
		. "${scriptCalled}" "${@}"
	}

	# First check to see if the script is an admin-only script or not, and only run it if the person is an admin if so.
	playerSpawnIni="$(echo "${scriptCalled}" | sed 's/\.sh$//g').ini"
	# echo "playerSpawnIni: ${playerSpawnIni}"
	runScript "$@"
else
	echo "Usage: playerSpawnLoader.sh \"\PATH\TO\SCRIPT\playerMessage.sh\" [PlayerName]"
fi
