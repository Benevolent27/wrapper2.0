...
if [ "${2}" ]; then
	...
	# echo "name: ${name}"
	shift
	echo "# newShipLoader Values:  name: ${name}"
	...
	function runScript {
		...
		echo "Running newShip script: ${scriptCalled}"
		. "${scriptCalled}" "${@}"
	}
	...
	echo 'Usage: newShipLoader.sh \"\PATH\TO\SCRIPT\newShip.sh\" [PlayerName] ["newShipName"] ("To Sector") ("BluePrint Name") (price)'
