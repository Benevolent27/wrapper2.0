...
if [ "${2}" ]; then
	...
	function runScript {
		...
		echo "Running playerDisconnect script: ${scriptCalled} ${@}"
		. "${scriptCalled}" "${@}"
	}

	# First check to see if the script is an admin-only script or not, and only run it if the person is an admin if so.
	playerDisconnectIni="$(echo "${scriptCalled}" | sed 's/\.sh$//g').ini"
	# echo "playerDisconnectIni: ${playerDisconnectIni}"
	runScript "$@"
else
	echo "Usage: playerDisconnectLoader.sh \"\PATH\TO\SCRIPT\playerDisconnect.sh\" [PlayerName]"
fi
