...
if [ "${2}" ]; then
	...
	if [ "${deathType}" == "person" -o "${deathType}" == "personInShip" ]; then
		killer="$1"
		shift
		if [ "$1" ]; then
			responsibleEntity="$1"
	...
	# I still need to work out asteroids, planets, stars, etc.



	function runScript {
		# This runs the script in-line so that it inherits the variables set above
		# "${scriptDir}log.sh" "DEBUGGING playerDeathLoader responsible entity: ${responsibleEntity}"
	...
	echo "Usage: playerDeathLoader.sh \"\PATH\TO\SCRIPT\playerDeath.sh\" [PlayerName] [deathType] (killer/Entity Responsible) (Entity Responsible)"
fi
